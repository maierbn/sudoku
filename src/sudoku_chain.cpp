...
srand(1);
  ...
  if (false)
  {
    ...
    sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 0, 0));   //0
    sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 0, 8));   //1
    sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, -3, 6, 4));   //2

    sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, -8, 0));  //3
    sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 0, -8, 5));  //4

    sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 0, -9, 6));  //5
    sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 0, 8, 7));   //6
  }

  // 2019 - 2
  sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 0, 0, 8));
  sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 8, 0, 9));
  sudokuSpecifications_.push_back(std::make_shared<SudokuSpecification>(Sudoku::SudokuType::classical, 3, 8, 0, 10));
  ...
  //sudokuSpecifications_[3]->previous = sudokuSpecifications_[2];
  ...
  for(auto &specification : sudokuSpecifications_)
  {
    ...
    if(specification->extra==4)
    {
      for (int i = 0; i < 6; i++)
      {
        fields_[getFieldIndex(0, 0, specification)]->addDependency(fields_[getFieldIndex(6, i, specification->previous)]);
        fields_[getFieldIndex(0, 1, specification)]->addDependency(fields_[getFieldIndex(6, i, specification->previous)]);
        fields_[getFieldIndex(0, 2, specification)]->addDependency(fields_[getFieldIndex(6, i, specification->previous)]);
      }

      for (int i = 0; i < 6; i++)
      {
        fields_[getFieldIndex(1, 0, specification)]->addDependency(fields_[getFieldIndex(7, i, specification->previous)]);
        fields_[getFieldIndex(1, 1, specification)]->addDependency(fields_[getFieldIndex(7, i, specification->previous)]);
        fields_[getFieldIndex(1, 2, specification)]->addDependency(fields_[getFieldIndex(7, i, specification->previous)]);
      }


      for (int i = 0; i < 6; i++)
      {
        fields_[getFieldIndex(2, 0, specification)]->addDependency(fields_[getFieldIndex(8, i, specification->previous)]);
        fields_[getFieldIndex(2, 1, specification)]->addDependency(fields_[getFieldIndex(8, i, specification->previous)]);
        fields_[getFieldIndex(2, 2, specification)]->addDependency(fields_[getFieldIndex(8, i, specification->previous)]);
      }
    }
    else if(specification->extra==5)
    {
      int values[9] = {5,6,8,1,4,3,9,7,2};

      for (int i = 0; i < 9; i++)
      {

        fields_[getFieldIndex(i, 0, specification)]->setValue(values[i]);
        fields_[getFieldIndex(i, 0, specification)]->setFixed(true);
      }
    }
    else if(specification->extra==6)
    {
      int values_top[9] = {7,1,3,2,9,4,6,5,8};
      int values_left[6] = {7,2,4,8,3,1};
      int values_bottom[9] = {5,6,7,2,3,1,9,4,8};

      for (int i = 0; i < 9; i++)
      {
        fields_[getFieldIndex(i, 0, specification)]->setValue(values_top[i]);
        fields_[getFieldIndex(i, 0, specification)]->setFixed(true);

        //fields_[getFieldIndex(i, 8, specification)]->setValue(values_bottom[i]);
        //fields_[getFieldIndex(i, 8, specification)]->setFixed(true);
      }

      for (int i = 0; i < 6; i++)
      {
        fields_[getFieldIndex(0, i, specification)]->setValue(values_left[i]);
        fields_[getFieldIndex(0, i, specification)]->setFixed(true);
      }
    }
    else if(specification->extra==7)
    {
      int values_left[9] = {5,9,8,4,2,7,1,3,6};
    ...
    else if(specification->extra==8)
    {
      int values_top[9] = {1,2,3,9,6,7,8,4,5};
    ...
    else if(specification->extra==9)
    {
      int values_top[9] = {5,4,1,3,8,9,2,7,6};
    ...
    else if(specification->extra==10)
    {
      int values_top[9] = {6,9,7,8,1,3,2,5,4};
...
void SudokuChain::generate2019()
{
  std::cout << "start generate2019" << std::endl;
  srand(1);

  // upper normal 3x3 sudoku
  int nNewFields = 81;
  int max_value = 9;
  std::vector<std::shared_ptr<Field>> currentSudokuFields;
  ...
  for(int i = 0; i < nNewFields; i++)
  {
    std::shared_ptr<Field> newField = std::make_shared<Field>(id_counter_++, max_value);
    fields_.push_back(newField);
    currentSudokuFields.push_back(newField);
  }

  // create sudoku with new pointers
  sudoku_.push_back(std::make_shared<ChainedSudoku>(currentSudokuFields, 3, Sudoku::SudokuType::classical, this));

  // lower normal 3x3 sudoku
  currentSudokuFields.clear();

  // copy last 3 3x3 blocks
  std::copy(fields_.begin()+6*9, fields_.end(), std::back_inserter(currentSudokuFields));
  ...
  // specification is done, now produce solution etc.
