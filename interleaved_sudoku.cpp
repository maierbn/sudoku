...
{ 
  ...
  for(int x = 0; x < squared_size_; x++)
  {
    ...
    {
      fields_[index(x,y,0)] = std::move(std::unique_ptr<Field>(new Field(x,y,0,squared_size_)));
      fields_[index(x,y,1)] = std::move(std::unique_ptr<Field>(new Field(x,y,1,squared_size_)));
    }
  }
